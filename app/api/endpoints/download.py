...
async def download_file_by_aweme_id(download_path: str,
                               aweme_id: str ,
                               drama_name: str =None,
                               data:json =None,
                               prefix: bool = False,
    ...
    try:
        if not data:
            data = await HybridCrawler.DouyinWebCrawler.fetch_one_video(aweme_id)
    except Exception as e:
        return False,str(e)
    ...
    try:
        ...
        if data_type == 'video':
            file_name = f"{file_prefix}{platform}_{aweme_id}.mp4" if not with_watermark else f"{file_prefix}{platform}_{aweme_id}_watermark.mp4"
            url = video_info_json.get('video').get('play_addr').get('url_list')[0]
            file_path = os.path.join(download_path, file_name)
            ...
            success = await fetch_data_stream(url, None, headers=__headers, file_path=file_path)
            if not success:
                raise HTTPException(
                    status_code=500,
                    detail="An error occurred while fetching data"
